...
class UserService
{
    ...
    public function getWebsiteSchema($article = null)
    {
        ...
        $graph->organization()
            ->name('charityBay')
            ->founders(\Spatie\SchemaOrg\Schema::person()->name('Haidar Al-fallouji'))
            ->sameAs(['https://www.charitybay.org','https://www.charitybay.net','https://www.charitybay.org.au','https://www.charitybay.net.au','https://www.donateitems.net','https://www.facebook.com/charityBayAU','https://twitter.com/charitybayAU','https://www.instagram.com/charitybayau','https://www.linkedin.com/company/charitybay','https://www.youtube.com/channel/UCUJAdJbWOSOSa0z7D-2z4VQ','https://apps.apple.com/au/app/charitybay-donate-buy-smile/id1479319627','https://abr.business.gov.au/ABN/View?id=60636169698','https://play.google.com/store/apps/details?id=com.charitybay'])
            ->legalName('CHARITYBAY PTY LTD')
            ->slogan('The new era of giving back')
            ...
            ->description('')
            ->taxID(60636169698)
            ->logo(($this->container->getParameter('uvdesk.site_url').'/'.$this->getWebsiteDetails('knowledgebase')->getLogo()))
            ->url($this->container->getParameter('uvdesk.site_url'))
            ->email('info@charitybay.org')
            ->duns(746822318)
            ->potentialAction(
                \Spatie\SchemaOrg\Schema::searchAction()
                    ->target((new \Spatie\SchemaOrg\EntryPoint)->urlTemplate($this->container->getParameter('uvdesk.site_url').'/en/search?s={search_term_string}'))
